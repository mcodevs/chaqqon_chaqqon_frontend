...
describe('normalizePracticeConfig', () => {
  it('keeps a valid config as it is', () => {
    const config = { section: 'katta', rowCount: 7, secondsPerNumber: 0.8, problemCount: 8, digitCount: 2 };
    expect(normalizePracticeConfig(config)).toEqual(config);
  });

  it('clamps numbers into the allowed limits', () => {
    expect(
      normalizePracticeConfig({ section: 'miks', rowCount: 50, secondsPerNumber: 0, problemCount: 6.6, digitCount: 5 }),
    ).toEqual({
      section: 'miks',
      rowCount: 10,
